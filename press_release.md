...
Loan default risk prediction is a well studied field, with complex models used to output a probability that individuals or businesses will be unable to pay back their loans in full. These probabilities are often used to determine whether loans are granted or denied. These decisions are crucial. A wrong prediction could mean economic disaster for banks, the SBA, and small business owners alike. Current models focus solely on aspects of the specific business and loan to determine risk which are the most important factors contributing to loan defaults. However, going beyond to examine larger context could separate banks that have good lending practices to ones that are at the top of the industry.
...
In order to tackle this problem we gathered data about 400k+ loans approved by the SBA with characteristic about the business and the loan itself. We also gathered economic information, such as unemployment rate and median income, at the state and national level at the time the loan was approved. We proceeded to build three different types of machine learning models trained on two datasets - one as a baseline including information about the loan and the business and one as a comparison including economic indicators. 
...
In conclusion, looking at state level economic context can improve ability to classify the default risk of loans. This improvement may seem small, but it is meaningful.  
## Charts
